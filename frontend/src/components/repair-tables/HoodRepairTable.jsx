...
export default function HoodRepairTable({ data = [] }) {
  return (
    <div className="w-full">
      <div className="flex items-center gap-3 mb-2">
        <HoodIcon className="w-7 h-7" />
        <span className="text-xl font-bold text-[#2840A1]">Hood</span>
          ...
          <tbody>
            {data.length === 0 ? (
              ...
              data.map((item, idx) => (
                ...
                >
                  <td
                    className="px-4 py-3 text-gray-700 bg-amber-200 font-semibold w-[250px] truncate"
                    title={item.category}
                  >
                    {item.category}
                  </td>
