...
router.post("/remove", (req, res) => {
  ...
  db.db.all(
    ...
    (err, existingItems) => {
      if (err) {
        console.log("Error removing inventory stock: ", err);
        return res.status(500).json({ error: "Internal Server Error" });
      }
      ...
      if (existingItems.length === 0) {
        return res.status(404).json({
          error:
            "Item not found. Cannot remove stock from a non-existent item.",
        });
      }
...
router.patch("/:id", (req, res) => {
  ...
  db.db.get("SELECT * FROM items WHERE id = ?", [id], (err, existingItem) => {
    if (err) {
      console.error("Error updating item quantity:", err);
      return res.status(500).json({ error: "Internal Server Error" });
    }
...
router.patch("/lend/:id", (req, res) => {
  ...
  db.db.all(
    ...
    (err, returnedItems) => {
      if (err) {
        console.error("Error processing lend request:", err);
        return res.status(500).json({ error: "Internal Server Error" });
      }
      ...
      db.db.run(
        "UPDATE items SET quantity = ? WHERE id = ?",
        [newReturnedQuantity, returnedItems[0].id],
        function (err) {
          if (err) {
            console.error("Error updating returned items:", err);
            return res.status(500).json({ error: "Internal Server Error" });
          }
          ...
          db.db.all(
            ...
            [item_type, variant],
            (err, notReturnedItems) => {
              if (err) {
                console.error("Error checking not returned items:", err);
                return res.status(500).json({ error: "Internal Server Error" });
              }

              if (notReturnedItems.length > 0) {
                // Increase the quantity of "Not Returned" items
                const newNotReturnedQuantity =
                  notReturnedItems[0].quantity + quantity;
                db.db.run(
                  "UPDATE items SET quantity = ? WHERE id = ?",
                  [newNotReturnedQuantity, notReturnedItems[0].id],
                  function (err) {
                    if (err) {
                      console.error("Error updating not returned items:", err);
                      return res
                        .status(500)
                ...
                db.db.run(
                  ...
                  function (err) {
                    if (err) {
                      console.error("Error creating not returned item:", err);
                      return res
                        .status(500)
                        .json({ error: "Internal Server Error" });
                    }
...
router.patch("/set-returned/:id", (req, res) => {
  const { id } = req.params;
  const { item_type, variant, quantity } = req.body;
  ...
  db.db.all(
    ...
    (err, notReturnedItems) => {
      if (err) {
        console.error("Error processing set returned request:", err);
        return res.status(500).json({ error: "Internal Server Error" });
      }
      ...
      db.db.run(
        ...
        function (err) {
          ...
          db.db.all(
            ...
            (err, returnedItems) => {
              if (err) {
                console.error("Error checking returned items:", err);
                return res.status(500).json({ error: "Internal Server Error" });
              }
              ...
              if (returnedItems.length > 0) {
                ...
                const newReturnedQuantity =
                  returnedItems[0].quantity + quantity;
                console.log(
                ...
                db.db.run(
                  ...
                  function (err) {
                    if (err) {
                      console.error("Error creating returned item:", err);
                      return res
                        .status(500)
                        .json({ error: "Internal Server Error" });
                    }
