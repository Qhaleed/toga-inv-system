...
router.post("/", async (req, res) => {
  ...
  try {
    // Check for existing email first (more detailed check)
    const existingUsers = await db.getTable(email);
    if (existingUsers.length > 0) {
      return res.status(400).json({ message: "Email already registered" });
    }

    // Check for existing ID number to prevent duplicate student IDs
    const existingIdNumber = await new Promise((resolve, reject) => {
      db.db.all(
        "SELECT account_id FROM accounts WHERE id_number = ? AND id_number IS NOT NULL AND id_number != ''",
        [idNumber],
        (err, rows) => {
          if (err) reject(err);
          else resolve(rows);
    ...
    if (!email || !email.endsWith("@adzu.edu.ph")) {
      return res
        .status(400)
        .json({ message: "Email must end with @adzu.edu.ph" });
    }
    ...
    const result = await db.registForm({
